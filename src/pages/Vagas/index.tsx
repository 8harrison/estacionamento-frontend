...
const Vagas = () => {
  ...
  const [filtroStatus, setFiltroStatus] = useState<StatusVaga>(StatusVaga.todos);
  ...
  const fetchVagas = async () => {
    try {
      ...
      setError('Não foi possível carregar a lista de vagas. Tente novamente mais tarde.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleQuickStatusChange = async (vaga: Vaga) => {
    try {
      ...
      setError('Não foi possível atualizar o status da vaga. Tente novamente mais tarde.');
    }
  };
  ...
  const filteredVagas = vagas.filter(vaga => {
    ...
    // Filtro por setor
    const matchesSetor = filtroSetor === 'todos' || vaga.setor === filtroSetor;
          ...
          {filteredVagas.map((vaga) => (
            ...
            >
              <div className={styles.vagaHeader}>
                ...
                <div className={styles.vagaTipoIcon}>{getTipoIcon(vaga.tipo)}</div>
              </div>
              ...
              <div className={styles.vagaActions}>
                ...
                <button 
                  className={`${styles.vagaStatusButton} ${vaga.ocupada ? styles.vagaOcuparButton : styles.vagaLiberarButton}`}
